...
# ETN + REN + energy v enem joinu — samo trznost=1
df = pd.read_sql("""
    SELECT
        ...
        ds.st_nadstropja, ds.visina_etaze_net, ds.visina_interpretation, ds.id_lega,
...
df['eur_m2'] = df['pogodbena_cena_odskodnina'] / df['prodana_povrsina']
df = df[df['eur_m2'].between(300,12000)]
...
df['st_etaz']    = pd.to_numeric(df['st_etaz'], errors='coerce')
df['leto_izg']   = pd.to_numeric(df['leto_izg_sta'], errors='coerce')
df['era']        = df['leto_izg'].apply(era)
df['floor_ratio']= (df['st_nads'].clip(0,20)/df['st_etaz'].clip(1,20)).clip(0,1)
...
df['sqrt_pov'] = np.sqrt(df['prodana_povrsina'])
...
FEATURES = ['heatmap_price','prodana_povrsina','sqrt_pov','era',
            'floor_ratio','ceil_h','ceil_unc','energy','renov']

df_c = df.dropna(subset=['price_ratio','heatmap_price','prodana_povrsina'])
...
test = pd.DataFrame([{'heatmap_price':hmp,'prodana_povrsina':80,'sqrt_pov':np.sqrt(80),
    'era':0,'floor_ratio':4/6,'ceil_h':3.25,'ceil_unc':0,'energy':2,'renov':0.1}])
adj = np.exp(model.predict(test)[0])
final = hmp*adj
print(f"\nGallusovo: {hmp:,} × {adj:.3f} = {final:,.0f} €/m² → {final*80:,.0f}€")
print(f"Oče ocena: 450k–600k€")

os.makedirs('/tmp/rer_model',exist_ok=True)
joblib.dump(model,'/tmp/rer_model/adjustment_model_v3.pkl')
print("\n✅ Model v3 shranjen")
