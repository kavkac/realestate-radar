...
Pobere DMP LiDAR tile-e iz CLSS CDN.
Token se avtomatsko bere iz DB (app_config) — refresh-clss-token.ts ga osvezi vsak dan.

Uporaba:
  python3 scripts/download-lidar-tiles.py --out-dir /data/lidar/dmp --workers 4
  python3 scripts/download-lidar-tiles.py --out-dir /data/lidar/dmp --limit 100  # test batch
  python3 scripts/download-lidar-tiles.py --out-dir /data/lidar/dmp --resume     # skip obstoječe
"""
...
def get_tiles(conn, limit=None):
    ...
    with conn.cursor() as cur:
        sql = "SELECT ti_name, link_dmp FROM dmr_download_urls WHERE link_dmp IS NOT NULL"
        if limit:
            sql += f" LIMIT {limit}"
        cur.execute(sql)
        return cur.fetchall()
...
def main():
    parser = argparse.ArgumentParser(description="Download CLSS DMP LiDAR tiles")
